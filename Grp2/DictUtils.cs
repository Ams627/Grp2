...
namespace Grp2
{
    ...
    internal class DictUtils
    {
        ...
        /// <typeparam name="T2">Type of the data stored in the List</typeparam>
        /// <param name="d">Dictionary to add to</param>
        /// <param name="key">Dictionary key of the HashSet to add to</param>
        /// <param name="listEntry">Value to add to the HashSet</param>
        public static void AddEntryToList<T1, T2>(Dictionary<T1, SortedSet<T2>> d, T1 key, T2 listEntry)
        {
            if (!d.TryGetValue(key, out var list))
            {
                list = new SortedSet<T2>();
                d.Add(key, list);
            }
